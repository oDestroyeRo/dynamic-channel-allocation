...
class Policy:
    ...
    def multi_channel_build_model(self, n_inputs, n_outputs):
        ...
        model.add(Conv2D(64, 2, strides=(1, 1), activation='relu', input_shape=n_inputs, padding="valid", data_format="channels_last"))
        model.add(Conv2D(128, 2, strides=(1, 1), activation='relu', padding="valid", input_shape=n_inputs, data_format="channels_last"))
        model.add(Conv2D(128, 2, strides=(1, 1), activation='relu', padding="valid", input_shape=n_inputs, data_format="channels_last"))
        model.add(Flatten())
